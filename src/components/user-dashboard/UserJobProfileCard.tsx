...
export function UserJobProfileCard({
  ...
  return (
    <article className="relative flex h-full flex-col rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-5 shadow-sm transition-all hover:border-primary/25 hover:shadow-md">
      <Link
      ...
      <div className="relative z-[2] flex min-h-0 flex-1 flex-col pointer-events-none">
        ...
        <h3 className="font-headline text-lg font-bold leading-snug text-on-surface line-clamp-2">{title}</h3>
        {hasKeywords && (
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-outline-variant/10 pt-4">
          ...
          <button
            type="button"
            onClick={onInterview}
            className="pointer-events-auto relative z-[3] inline-flex shrink-0 items-center justify-center gap-1 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-on-primary shadow-sm transition hover:opacity-95 active:scale-[0.98]"
          >
            {interviewCta}

...
export function PracticeModes({
  ...
  const modes = [
    { key: "chat", Icon: MessageSquare, tone: "bg-[#E8F5E9] text-[#2E7D32]", duration: "10–15", level: "userDash.mode.chat.level", action: () => onNavigate("/chat") },
    { key: "voice", Icon: Mic, tone: "bg-[#FFEBEE] text-[#D32F2F]", duration: "5–15", level: "userDash.mode.voice.level", action: () => onNavigate("/voice") },
    { key: "video", Icon: Video, tone: "bg-[#FCB625] text-[#234196]", duration: "20–30", level: "userDash.mode.video.level", action: onStartVideo },
  ] as const;
      ...
      <div className="mb-5 flex items-end justify-between gap-4">
        ...
        {sessions.length ? (
          <button type="button" onClick={() => onNavigate("/interview/select")} className="hidden min-h-11 items-center gap-2 text-sm font-bold underline decoration-[#FCB625] decoration-4 underline-offset-4 sm:inline-flex">
            {t("userDash.activity.viewAll")}<ArrowRight className="size-4" aria-hidden="true" />
          </button>
        ) : null}
      </div>
      {sessions.length ? (
            ...
            return (
              <article key={session.roomId} className="grid gap-4 p-5 sm:grid-cols-[minmax(0,1.5fr)_minmax(7rem,.6fr)_auto] sm:items-center">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border-2 border-[#234196] bg-[#FCB625] text-[#234196]"><Icon className="size-5" aria-hidden="true" /></span>
        ...
        <div className={`${surface} bg-[#F0F4FC] p-8 text-center`}>
          <History className="mx-auto size-8 text-[#234196]" aria-hidden="true" />
...
export function LearningResources({ onNavigate, t }: { onNavigate: (href: string) => void; t: Translate }) {
  const resources = [
    { href: "/practice", Icon: HelpCircle, tone: "bg-[#E8F5E9] text-[#2E7D32]", title: "userDash.learning.quizTitle", body: "userDash.learning.quizBody", cta: "userDash.learning.quizCta" },
    { href: "/resources", Icon: BookOpen, tone: "bg-[#FCB625] text-[#234196]", title: "userDash.learning.resourcesTitle", body: "userDash.learning.resourcesBody", cta: "userDash.learning.resourcesCta" },
  ];
  return (
        ...
        <div className="grid gap-0 bg-[#234196] sm:grid-cols-2 lg:col-span-7">
          {resources.map((resource) => (
            <button key={resource.href} type="button" onClick={() => onNavigate(resource.href)} className="group m-px min-h-44 bg-white p-6 text-left hover:bg-[#FEF9EE]">
              <span className={`inline-flex rounded-lg border-2 border-[#234196] p-2 ${resource.tone}`} aria-hidden="true"><resource.Icon className="size-5" /></span>
              <h3 className="mt-4 font-headline text-xl">{t(resource.title)}</h3>
              <p className="mt-2 text-sm text-[#5A6B8F]">{t(resource.body)}</p>
              <span className="mt-4 inline-flex items-center gap-1 text-sm font-bold underline decoration-[#FCB625] decoration-4 underline-offset-4">{t(resource.cta)}<ArrowRight className="size-4 transition-transform duration-200 group-hover:translate-x-1 motion-reduce:transition-none" aria-hidden="true" /></span>
            </button>
          ))}
        </div>
      </div>

...
export type MascotProps = {
  ...
  label?: string
  /** Optional active reaction override. */
  reaction?: Reaction | null
}

export function Mascot(props: MascotProps) {
  const { directions, reactions, size = 140, className, label = 'mascot', reaction: propReaction } = props
  ...
  const [internalReaction, setInternalReaction] = useState<Reaction | null>(null)
  const reaction = propReaction !== undefined ? propReaction : internalReaction
  ...
  const boop = () => {
    ...
    const later = (ms: number, next: Reaction | null) => {
      timersRef.current.push(window.setTimeout(() => setInternalReaction(next), ms))
    }

...
export const QuestionTimerBadge: React.FC<QuestionTimerBadgeProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Icon đồng hồ */}
      {isTimingOut ? (
        <Hourglass className={`size-4 transition-transform ${isAlert ? "scale-110 text-[#D32F2F]" : "text-[#234196]"}`} aria-hidden="true" />
      ) : (
        <Clock className={`size-4 transition-transform ${isAlert ? "scale-110 text-[#D32F2F]" : "text-[#234196]"}`} aria-hidden="true" />
      )}
